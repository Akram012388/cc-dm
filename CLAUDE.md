...
Session identity: `id` is always auto-generated as `session-<12 hex chars>` using `crypto.randomUUID()`. Display `name` comes from `CC_DM_SESSION_NAME` env var (falls back to `CC_DM_SESSION_ID` for backward compat, then to the auto-generated id). All names are sanitized (lowercase, trimmed, spaces→hyphens) at both server startup and tool invocation. `role` comes from `CC_DM_SESSION_ROLE` (defaults to `worker`). `project` comes from `CC_DM_SESSION_PROJECT` (defaults to `''`). `cwd` is captured from `process.cwd()` at registration. Session names are unique — `handleRegister` rejects names already taken by a different session ID.
